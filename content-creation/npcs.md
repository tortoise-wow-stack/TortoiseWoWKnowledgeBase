...
**Vendor:** `npc_vendor` (entry, slot, item, maxcount/incrtime = limited stock, itemflags, condition_id) — or `npc_vendor_template` via `trainer_id`/`vendor_id` in template. **Trainer:** `npc_trainer` (entry, spell, spellcost, reqskill, reqskillvalue, reqlevel) or `npc_trainer_template`. **Quest giver/turn-in:** `creature_questrelation` / `creature_involvedrelation` (id, quest).
